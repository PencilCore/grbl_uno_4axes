...
static inline void st_write_step_ports()
{
  uint8_t portd_bits = 0;
  uint8_t portb_bits = 0;

  // 映射全局位号到 PORTD 局部位
  if (st.step_outbits & (1<<X_STEP_BIT)) portd_bits |= (1<<X_STEP_BIT);
  if (st.step_outbits & (1<<Y_STEP_BIT)) portd_bits |= (1<<Y_STEP_BIT);
  if (st.step_outbits & (1<<Z_STEP_BIT)) portd_bits |= (1<<Z_STEP_BIT);

  // 映射全局位号到 PORTB 局部位
  if (st.step_outbits & (1<<Q_STEP_BIT)) portb_bits |= (1<<Q_STEP_BIT );

  // 写端口
  STEP_PORT  = (STEP_PORT & ~STEP_MASK1) | portd_bits;
  STEP_PORTQ = (STEP_PORTQ & ~STEP_MASKQ) | (portb_bits << 4);
}

static void st_write_direction_ports()
{
    uint8_t portd_bits = 0;
    uint8_t portb_bits = 0;

    // PORTD 方向位
    if (st.dir_outbits & (1<<X_DIRECTION_BIT)) portd_bits |= (1<<X_DIRECTION_BIT);
    if (st.dir_outbits & (1<<Y_DIRECTION_BIT)) portd_bits |= (1<<Y_DIRECTION_BIT);
    if (st.dir_outbits & (1<<Z_DIRECTION_BIT)) portd_bits |= (1<<Z_DIRECTION_BIT);

    // PORTB 方向位 (只包含 Q/U)
    if (st.dir_outbits & (1<<Q_DIRECTION_BIT)) portb_bits |= (1<<Q_DIRECTION_BIT);

    DIRECTION_PORT  = (DIRECTION_PORT & ~DIRECTION_MASK1) | portd_bits;
    DIRECTION_PORTQ = (DIRECTION_PORTQ & ~DIRECTION_MASKQ) | portb_bits;
}
...
ISR(TIMER1_COMPA_vect)
{
  ...
  DIRECTION_PORT  = (DIRECTION_PORT  & ~DIRECTION_MASK1) | (st.dir_outbits & DIRECTION_MASK1);
  DIRECTION_PORTQ = (DIRECTION_PORTQ & ~DIRECTION_MASKQ) | ((st.dir_outbits << 4) & DIRECTION_MASKQ);
  ...
  // Then pulse the stepping pins
    ...
    STEP_PORT = (STEP_PORT & ~STEP_MASK1) | st.step_outbits;
    STEP_PORTQ = (STEP_PORTQ & ~STEP_MASKQ) | (st.step_outbits << 4);
  #endif
  ...
  if (st.exec_segment == NULL) {
    ...
    if (segment_buffer_head != segment_buffer_tail) {
      ...
      #ifdef AXIS_Q_EXIST
              st.counter_x = st.counter_y = st.counter_z = st.counter_q = (st.exec_block->step_event_count >> 1);
      #else
...
ISR(TIMER0_OVF_vect)
{
  uint8_t step_bits_d = (STEP_PORT & ~STEP_MASK1) | (st.step_outbits & STEP_MASK1); // XYZ
  uint8_t step_bits_b = (STEP_PORTQ & ~STEP_MASKQ) | ((st.step_outbits << 4) & STEP_MASKQ); // Q

  /* 同一周期写回两个端口 *********************************************/
  STEP_PORT  = step_bits_d;   // PORTD 只改 XYZ 位
  STEP_PORTQ = step_bits_b; 
...
void st_reset()
{
  ...
  uint8_t step_bits_d = (STEP_PORT  & ~STEP_MASK1) | (step_port_invert_mask & STEP_MASK1);
  uint8_t step_bits_b = (STEP_PORTQ & ~STEP_MASKQ) | (step_port_invert_mask & STEP_MASKQ);

  STEP_PORT  = step_bits_d;   // PORTD: 只改 XYZ
  STEP_PORTQ = step_bits_b;   // PORTB: 只改 Q

  DIRECTION_PORT  = (DIRECTION_PORT  & ~DIRECTION_MASK1) | (dir_port_invert_mask & DIRECTION_MASK1);
  DIRECTION_PORTQ = (DIRECTION_PORTQ & ~DIRECTION_MASKQ) | (dir_port_invert_mask & DIRECTION_MASKQ);
...
void stepper_init()
{
  ...
  STEP_DDR  |= STEP_MASK1;
  STEP_DDRQ |= STEP_MASKQ; 
  STEPPERS_DISABLE_DDR |= 1<<STEPPERS_DISABLE_BIT;
  DIRECTION_DDR |= DIRECTION_MASK1;
  DIRECTION_DDRQ |= DIRECTION_MASKQ;
