...
#define SLEEP_SEC_PER_OVERFLOW (65525.0*64.0/F_CPU) // With 16-bit timer size and prescaler
...
void sleep_init()
{
  // Configure Timer 2: Sleep Counter Overflow Interrupt
  ...
  // TCCR2B |= (1<<CS22); // Enable timer with 1/256 prescaler. ~4.4min max with uint8 and 1.05sec/tick
  // TCCR2B |= (1<<CS21); // Enable timer with 1/8 prescaler. ~8.2sec max with uint8 and 22.7msec/tick
  TCCR2B |= (1<<CS21)|(1<<CS20); // Enable timer with 1/64 prescaler. ~66.8sec max with uint8 and 0.262sec/tick
  // TCCR2B |= (1<<CS22)|(1<<CS20); // Enable timer with 1/1024 prescaler. ~17.8min max with uint8 and 4.19sec/tick
  sleep_disable();
}
