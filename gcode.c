    ...
    switch(letter) {
        ...
        switch(int_value) {
          ...
          case 4: case 53:
            ...
            if ((int_value == 28) || (int_value == 30) || (int_value == 92)) {
              if (!((mantissa == 0) || (mantissa == 10))) { FAIL(STATUS_GCODE_MODAL_GROUP_VIOLATION); }
          ...
          case 80:
            ...
            if (int_value == 38){
              if (!((mantissa == 20) || (mantissa == 30) || (mantissa == 40) || (mantissa == 50))) {
                FAIL(STATUS_GCODE_MODAL_GROUP_VIOLATION); // [Unsupported G38.x command]
              }
          ...
          case 90: case 91:
            if (mantissa == 0) {
              ...
              if ((mantissa != 10) || (int_value == 90)) { FAIL(STATUS_GCODE_MODAL_GROUP_VIOLATION); } // [G90.1 not supported]
              mantissa = 0; // Set to zero to indicate valid non-integer G command.
          ...
          case 43: case 49:
            ...
            } else { FAIL(STATUS_GCODE_MODAL_GROUP_VIOLATION); } // [Unsupported G43.x command]
            mantissa = 0; // Set to zero to indicate valid non-integer G command.
            break;
          ...
          case 61:
            ...
            if (mantissa != 0) { FAIL(STATUS_GCODE_MODAL_GROUP_VIOLATION); } // [G61.1 not supported]
            // gc_block.modal.control = CONTROL_MODE_EXACT_PATH; // G61
            break;
          default: FAIL(STATUS_GCODE_MODAL_GROUP_VIOLATION); // [Unsupported G command]
        }
      ...
      case 'M':
        ...
        switch(int_value) {
          ...
          default: FAIL(STATUS_GCODE_MODAL_GROUP_VIOLATION); // [Unsupported M command]
        }
...
/// +Q  linear axis for work piece
		...
		#endif
///
          default: FAIL(STATUS_GCODE_MODAL_GROUP_VIOLATION);
        }
    ...
    case NON_MODAL_SET_COORDINATE_DATA:
      ...
      if (gc_block.values.l != 20) {
        if (gc_block.values.l == 2) {
          if (bit_istrue(value_words,bit(WORD_R))) { FAIL(STATUS_GCODE_MODAL_GROUP_VIOLATION); } // [G10 L2 R not supported]
        } else { FAIL(STATUS_GCODE_MODAL_GROUP_VIOLATION); } // [Unsupported L]
      }
